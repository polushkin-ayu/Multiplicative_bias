...
def woe_line(df, score, target, n_buckets=5):
        ...
        plt.errorbar(df_grouped.mean_score, df_grouped.woe, yerr=df_grouped.se_woe,
                     label = 'WoE by bucket',color = colors_dict['lightblue'],fmt = 'o')

        plt.plot(df_grouped.mean_score, logit(y_preds[:,1]), color = colors_dict['red'], label = 'Logistic regression')
...
def plot_line(df, feature_nm, target_nm, n_buckets = 10):
        ...
        plt.plot(x, regression_line, label='regression_line', color = colors_dict['red'])
        plt.legend()
        plt.show()
